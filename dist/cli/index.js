...
import { hubLogin, hubLogout, hubListModels, hubUsage } from "../auth/hub.js";
...
if (nonFlagArgs[0] === "login" || nonFlagArgs[0] === "logout") {
    subcommand = nonFlagArgs.shift();
}
if (nonFlagArgs.length > 0) {
    initialPrompt = nonFlagArgs.join(" ");
}
// ─── Mode ──────────────────────────────────────────────
const MODE_ORDER = ["safe", "auto-approve", "yolo"];
const MODE_INFO = {
...
async function showSelector(items, initialIndex = 0) {
    return new Promise((resolve) => {
        ...
        function render() {
            if (drawn)
                process.stdout.write(`\x1b[${count}A`); // 블록 맨 위로
...
process.stdin.on("keypress", (_str, key) => {
    if (selectorActive || authInputActive)
        return;
...
function askYesAlwaysNo(question) {
    return new Promise((resolve) => {
        rl.question(question, (answer) => {
            const lower = answer.trim().toLowerCase();
...
async function showUsage() {
    ...
    try {
        ...
        const LABEL = { daily: "오늘", weekly: "이번 주", monthly: "이번 달" };
        ...
        for (const key of ["daily", "weekly", "monthly"]) {
            ...
            console.log("  " + chalk.cyan(LABEL[key].padEnd(7)) +
 ...
 */
async function loginFlow(cancellable = false) {
    console.log("");
    console.log(chalk.bold("  사내 계정으로 로그인"));
    console.log(chalk.dim(`  HUB: ${config.hubUrl}`));
    if (cancellable)
        console.log(chalk.dim("  빈 이메일 입력으로 취소"));
    console.log("");
    while (true) {
...
// ─── Agent Events ──────────────────────────────────────
async function processAgentEvents(gen) {
