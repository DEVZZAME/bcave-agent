...
const apple = `[디자인 프로필: Apple] — 차분·정밀·절제·넓은 여백·낮은 시각 소음 (macOS형 데스크톱 대시보드/분석 도구에 적합).
토큰(:root[data-design-system="apple"] 로 정의):
  --ds-bg:#f5f5f7; --ds-surface:#ffffff; --ds-text:#1d1d1f; --ds-text-secondary:#6e6e73;
  --ds-border:rgba(0,0,0,.10); --ds-accent:#0071e3; --ds-positive:#248a3d; --ds-warning:#b25000; --ds-negative:#d70015;
  --ds-radius-sm:8px; --ds-radius-md:12px; --ds-radius-lg:16px;
레이아웃: 좌측 사이드바 + 상단 툴바 + 콘텐츠의 계층을 명확히. 8px 간격 체계. 카드로 다 감싸지 말고 여백·얇은 separator·배경 명도차로 그룹 구분. 툴바가 콘텐츠보다 시각적으로 앞서지 않게.
타이포: 큰 제목보다 정교한 크기·굵기 차이를 사용. 본문 14px 이상. 숫자엔 단위·비교 기준 병기.
...
const microsoftFluent = `[디자인 프로필: Microsoft Fluent 2] — 생산성·높은 정보 밀도·명확한 계층·조용한 표면 (사내 업무 시스템/관리자·데이터 테이블 중심에 적합).
토큰(:root[data-design-system="microsoft-fluent"] 로 정의):
  --ds-bg:#f5f5f5; --ds-surface:#ffffff; --ds-surface-subtle:#fafafa; --ds-text:#242424; --ds-text-secondary:#616161;
  --ds-border:#d1d1d1; --ds-accent:#0f6cbd; --ds-positive:#0e7a0d; --ds-warning:#f7630c; --ds-negative:#c50f1f;
...
const toss = `[디자인 프로필: Toss-inspired] — 쉬움·명확함·친절함·행동 중심·강한 핵심 위계 (모바일 중심 생활/금융·쉬운 UX 에 적합). ※ Toss 브랜드·화면 복제 금지, 공식 TDS 라고 표현하지 말 것.
토큰(:root[data-design-system="toss"] 로 정의):
  --ds-bg:#f7f8fa; --ds-surface:#ffffff; --ds-text:#191f28; --ds-text-secondary:#6b7684; --ds-text-tertiary:#8b95a1;
  --ds-border:#e5e8eb; --ds-accent:#3182f6; --ds-accent-soft:#e8f3ff; --ds-positive:#00a878; --ds-warning:#f59f00; --ds-negative:#f04452;
  --ds-radius-sm:8px; --ds-radius-md:12px; --ds-radius-lg:16px;
...
타이포: 한국어 가독성 최우선. 큰 핵심 숫자 + 짧은 제목 + 읽기 쉬운 본문. 굵기를 과하게 다양화하지 말 것.
...
const kakao = `[디자인 프로필: Kakao-inspired] — 친근함·즉시 이해·생활 서비스다운 실용성·부드러운 형태·따뜻한 중립색 (생활 밀착/커뮤니케이션·B2C 에 적합). ※ Kakao 브랜드·말풍선·캐릭터·특정 화면 복제 금지, 공식 디자인 시스템이라 표현하지 말 것.
토큰(:root[data-design-system="kakao"] 로 정의):
  --ds-bg:#f7f7f5; --ds-surface:#ffffff; --ds-surface-warm:#faf9f6; --ds-text:#222222; --ds-text-secondary:#666666;
  --ds-border:#e6e6e6; --ds-accent:#fee500; --ds-accent-text:#191919; --ds-positive:#2e8b57; --ds-warning:#e68a00; --ds-negative:#e5484d;
  --ds-radius-sm:8px; --ds-radius-md:12px; --ds-radius-lg:16px;
정보구조: 자주 하는 행동(탐색·알림·메시지·최근 기록)을 첫 화면에서 빠르게. 제목·주요 행동은 단순하게. 친근함을 위해 정보 위계를 희생하지 말 것.
