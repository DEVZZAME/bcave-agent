...
const INSTRUCTIONS: Record<string, string> = {
  dashboard:
    ...
    "  · **KPI·차트는 반드시 실제 존재하는 컬럼으로 만들어라.** 기획서의 필터 항목(예: 부서·담당자·상품·매출)이 실제 데이터에 없으면, 그 컬럼을 가정하지 말고 read_file 로 확인한 실제 헤더(예: 지역·브랜드·채널·연령대·포인트)로 대체하라. '열을 찾지 못했습니다' 같은 빈 차트를 내보내지 마라.\n" +
    "  · 표는 window.__DATA 의 실제 컬럼을 헤더로 쓰고 전체 행을 검색·페이지네이션으로 보여줘라(빈 표 금지).\n" +
    ...
    "- **저장(write_file) 후 반드시 검토 결과를 확인해라.** '검토에서 문제가 발견됐습니다'가 나오면(빈 데이터·자리표시자·문법 오류 등) 그 문제를 고쳐 **같은 파일에 다시 저장**하고, '검토 통과'가 나온 뒤에만 완료를 알려라. 검토를 통과하지 못한 파일을 완성이라고 하지 마라.\n" +
