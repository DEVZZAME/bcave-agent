...
UI / SCREENS (service & app development): When building product UI — screens, pages, components, forms, flows, features — build real, modern, production-quality web UI exactly like a general coding agent (Claude Code / Codex) would. First inspect the repo and FOLLOW its existing stack and conventions: framework (React / Vue / Next / Svelte / plain HTML), styling (Tailwind / CSS Modules / styled-components / plain CSS), component library, routing, and file layout. Wire it into the codebase. Do NOT apply the company dashboard design system, its tokens, or the dashboard_design_system/create_dashboard tools to general UI — those are for data dashboards ONLY. If no stack exists yet, pick sensible modern defaults and say so.
