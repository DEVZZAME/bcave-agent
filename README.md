...
### 3단계. bcave 설치 — 아래 **둘 중 하나**

**방법 A ⭐ 권장 — Windows cmd·PowerShell 포함 어디서나**

터미널(cmd / PowerShell / 터미널.app 아무거나)에:

```bash
npm install -g github:DEVZZAME/bcave-code
```

→ 설치 후 바로 `bcave` 명령을 쓸 수 있습니다.

**방법 B — Mac / Linux / Git Bash 용 (스크립트)**
...
> **업데이트(최신 버전 받기)**: 방법 A는 위 `npm install -g ...` 명령을 **다시 실행**, 방법 B는 스크립트를 다시 실행하면 됩니다.
...
| Windows cmd·PowerShell에서 `bcave` 안 됨 | **방법 A(npm)** 로 설치했는지 확인. 방법 B(스크립트)는 Git Bash 전용 |
| 설치 중 `git: command not found` | 2단계 **Git 설치** 필요 |
