...
> 💡 **뭘 만들지 막막하면 `/kickstart`** — 개발을 몰라도 질문에 답하면 정리해서 **실제 결과물까지** 만들어줍니다. (질문 단계는 AI를 안 써서 무료)
...
**✨ 만들기 도우미 — 개발을 몰라도 괜찮아요**

| 명령 | 설명 |
|---|---|
| `/kickstart` | 질문에 답하면 정리 → **바로 결과물 생성** (질문 수집은 토큰 0. 대시보드는 백엔드 없는 예쁜 HTML). show/edit/reset/resume |
| `/ppt` | 발표자료(PPT)를 대화로 만들어줍니다 (실제 .pptx 파일) |
| `/dashboard` | 데이터를 한눈에 보는 대시보드를 만들어줍니다 |
| `/explain` | 이 프로젝트가 어떻게 돌아가는지 쉬운 비유로 설명 |
| `/redesign` | 기능은 그대로, 디자인만 새 느낌으로 |
| `/suggest` | 추가하면 좋을 기능을 제안 |
| `/deploy` | 만든 걸 인터넷에 올려 누구나 볼 수 있게 |
